...
from lib.datasets.kitti_dataset_nuscenes_attack import KittiDataset
...
class KittiRCNNDataset(KittiDataset):
    def __init__(self, root_dir, npoints=16384, split='train', classes='Car', mode='TRAIN', random_select=False,
    ...
    def get_image(self, idx):
        return super().get_image(idx % 200000)

    def get_image_shape(self, idx):
        return super().get_image_shape(idx % 200000)

    def get_calib(self, idx):
        return super().get_calib(idx % 200000)

    def get_road_plane(self, idx):
        return super().get_road_plane(idx % 200000)
    ...
    def get_rpn_sample(self, index):
        ...
        if sample_id < 200000:
            ...
            pose_lidar = self.get_pose(sample_id)
            # get valid point (projected points should be in image)
            pts_rect = calib.lidar_to_rect(pts_lidar[:, 0:3])
            pts_intensity = pts_lidar[:, 3]
        else:
            calib = self.get_calib(sample_id % 200000)
            # img = self.get_image(sample_id % 200000)
            img_shape = self.get_image_shape(sample_id % 200000)
        ...
        #     sample_info['pose_lidar'] = pose_lidar
        #     return sample_info
    ...
    #         # get valid point (projected points should be in image)
    #         pts_rect, lidar_to_rect_matrix = calib.lidar_to_rect(pts_lidar[:, 0:3])
    #         pts_intensity = pts_lidar[:, 3]
    #     else:
    #         calib = self.get_calib(sample_id % 200000)
    #
    ...
    #
    #         ret_pts_rect = pts_rect[choice, :]
    #         pts_lidar = pts_lidar[choice, :]
    #         ret_pts_intensity = pts_intensity[choice] - 0.5  # translate intensity to [-0.5, 0.5]
    #     else:
    #         ret_pts_rect = pts_rect
    #         ret_pts_intensity = pts_intensity - 0.5
    #
    #     ############################################################ Added by Yiming, 2020.10.22
    #     pose_matrix = np.squeeze(pose_lidar)
    #
    #     # we need to convert point cloud to nuscenes coordinate, and finally convert it back to kitti coordinate
    ...
    #         temp_pc = pts_lidar.T
    #         start_pc = np.dot(kitti_to_nu_lidar.rotation_matrix, temp_pc[:3,
    #                                                              :])  # <------------------be careful here! we need to firstly convert to nuscene format
    #
    #         # plt.scatter(start_pc[0, :], start_pc[2, :], s=0.2, c='g', alpha=1)
    #
    #         # change to polar coordinate
    #         polar_points = np.arctan2(start_pc[1, :], start_pc[0, :]) * 180 / np.pi + 180  # in degrees (0, 360]
    #
    #         start_pose_rec_translation = [pose_matrix[0, 0], pose_matrix[0, 1], pose_matrix[0, 2]]
    #         start_pose_rec_rotation = [pose_matrix[0, 3], pose_matrix[0, 4], pose_matrix[0, 5], pose_matrix[0, 6]]
    #
    #         start_cs_rec_translation = [pose_matrix[1, 0], pose_matrix[1, 1], pose_matrix[1, 2]]
    #         start_cs_rec_rotation = [pose_matrix[1, 3], pose_matrix[1, 4], pose_matrix[1, 5], pose_matrix[1, 6]]
    #
    #         end_pose_rec_translation = [pose_matrix[2, 0], pose_matrix[2, 1], pose_matrix[2, 2]]
    #         end_pose_rec_rotation = [pose_matrix[2, 3], pose_matrix[2, 4], pose_matrix[2, 5], pose_matrix[2, 6]]
    #
    #         ###### enable motion distortion
    #         # Init
    #         sensor_from_vehicle = transform_matrix(start_cs_rec_translation, Quaternion(start_cs_rec_rotation),
    #                                                inverse=True)
    #         vehicle_from_global = transform_matrix(start_pose_rec_translation, Quaternion(start_pose_rec_rotation),
    #                                                inverse=True)
    #
    #         global_from_car = transform_matrix(start_pose_rec_translation, Quaternion(start_pose_rec_rotation),
    #                                            inverse=False)
    #         car_from_current = transform_matrix(start_cs_rec_translation, Quaternion(start_cs_rec_rotation),
    #                                             inverse=False)
    #
    #         # find the next sample data
    #         translation_step = (np.array(end_pose_rec_translation) - np.array(start_pose_rec_translation)) / num_step
    #
    ...
    #         # trans_matrix_gps_tensor=torch.Tensor().cuda(non_blocking=True).float()
    #
    ...
    #             q_current = []
    #
    #             cosa = p_start[0] * q_end[0] + p_start[1] * q_end[1] + p_start[2] * q_end[2] + p_start[3] * q_end[3]
    #
    ...
    #
    #             ref_from_car = transform_matrix(start_cs_rec_translation, Quaternion(start_cs_rec_rotation),
    #                                             inverse=True)
    #             car_from_global = transform_matrix(t_current, Quaternion(q_current), inverse=True)
    #
    #             # select the points in a small scan area
    #             scan_start = polar_points > 360 * t / num_step
    #             scan_end = polar_points <= 360 * (t + 1) / num_step
    ...
    #             '''
    #             # t_current_distor = t_current + random.gauss(mu,sigma)
    #             # q_current_distor = np.array(q_current) + random.gauss(0,0.5)
    #             vehicle_from_sensor = transform_matrix(start_cs_rec_translation, Quaternion(start_cs_rec_rotation),
    #                                                    inverse=False)
    #             global_from_vehicle = transform_matrix(t_current, Quaternion(q_current), inverse=False)
    #             # can also calculate the inverse matrix of trans_matrix
    #             trans_matrix_gps = reduce(np.dot, [sensor_from_vehicle, vehicle_from_global, global_from_vehicle,
    #                                                vehicle_from_sensor])
    #
    ...
    #             else:
    #                 trans_matrix_gps_tensor = np.concatenate([trans_matrix_gps_tensor, trans_matrix_gps],
    #                                                          0)  # [1000, 4, 4]
    ...
    #         sample_info['pts_features'] = ret_pts_features
    #         sample_info['gt_boxes3d'] = aug_gt_boxes3d
    #         sample_info['pc_timestap_list'] = pc_timestap_list
    #         sample_info['trans_matrix_gps_tensor'] = trans_matrix_gps_tensor
    #         sample_info['lidar_to_rect_matrix'] = lidar_to_rect_matrix
    #         sample_info['pose_matrix'] = pose_matrix
    #
    #         return sample_info
    #     else:
    #         sample_info['pts_input'] = pts_input
    ...
    #         sample_info['pts_features'] = ret_pts_features
    #         sample_info['gt_boxes3d'] = aug_gt_boxes3d
    #         sample_info['pc_timestap_list'] = 0
    #         sample_info['trans_matrix_gps_tensor'] = 0
    #         sample_info['lidar_to_rect_matrix'] = 0
    #         sample_info['pose_matrix'] = pose_matrix
    #
    #         return sample_info
